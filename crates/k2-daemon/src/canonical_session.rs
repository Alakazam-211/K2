...
pub fn ensure_canonical_session(project_path: &str) -> Result<EnsureOutcome, String> {
    ...
    // #70: DB-canonical name (persona file `name:` → workspace basename when
    // the workspace is configured but has no AGENT.md) so the explicit
    // ensure-canonical-session path works for fileless configured workspaces.
