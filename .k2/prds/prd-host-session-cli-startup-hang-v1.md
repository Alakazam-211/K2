...
| H4 | **Auto-action v1:** full teardown via **`v2_session_map::unregister`** (same as `/kill`, not bare `sess.kill()` only) and surface a loud failure the integrator can retry on — *or* leave kill to Scout reconciler once Stage 2 is live. Prefer **daemon optional auto-kill** gated by env default **off** until proven; **integrator kill route remains SSOT**. See also `prd-host-session-completion-lifecycle-v1.md` for happy-path Grace full reap. |
