...
fn mint_bind_serve(pane: &str) -> (SessionId, String, PathBuf) {
    ...
    cell_server::serve_cell(sid, listener, None);
    (sid, token, sock)
}
...
async fn mail_manage_toggle_gates_cell_uds_m5() {
    ...
    let (ra_status, ra_body) =
        uds(&sock, &get("/cli/mail/server/rotate-admin", Some(&token))).await;
    assert_eq!(ra_status, 405, "UDS GET rotate-admin 405; {ra_body}");
}
