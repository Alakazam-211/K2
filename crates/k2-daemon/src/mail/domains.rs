...
impl DomainEngine for StalwartClient {
    fn create_domain(&self, domain: &str) -> Result<CreatedDomain, String> {
        let mut created = self.domain_create(domain)?;
        // ✔ LIVE-VERIFIED v0.16.10: the create reply carries only the
        // id; DKIM keys are generated by a background task the create
        // schedules, and the computed dnsZoneFile grows its
        // `_domainkey` rows ~1 s later. Bounded poll so add_domain
        // returns a COMPLETE record table; on timeout the last zone
        // (sans DKIM) is returned and the daily verification re-check
        // catches the rows later.
        for attempt in 0..20 {
            match self.domain_dns_zonefile(&created.id) {
                Ok(zone) => {
                    // BOTH default algorithms (Ed25519 lands first,
                    // RSA a beat later — live-observed): wait for two
                    // `_domainkey` rows, settle for one at timeout.
                    let dkim_rows = zone.matches("_domainkey").count();
                    created.dns_zone_file = Some(zone);
                    if dkim_rows >= 2 {
                        break;
                    }
                }
                // The very first get can race the create commit — keep
                // polling; add_domain falls back to dns_zonefile() and
                // fails loudly there if the domain truly can't be read.
                Err(_) => {}
            }
            if attempt + 1 < 20 {
...
/// [`super::secrets::FileSecretStore`] resolves ALL THREE ref forms —
/// the daemon-minted `mailsec_*` refs the enable machine writes into
/// the row (the live-verified real case), plus the operator-supplied
/// `env:<VAR>` / absolute-path spellings. A resolvable-but-absent ref
/// is "re-run enable", not a scheme error.
use super::secrets::{FileSecretStore, SecretStore as _};
...
pub fn engine_from_db() -> Result<(StalwartClient, Option<String>), String> {
    ...
    let api_key = FileSecretStore::default()
        .resolve(&key_ref)?
        .ok_or_else(|| {
            "mail server API key missing from the secret store — re-run Enable in \
             Settings → Email"
                .to_string()
        })?;
