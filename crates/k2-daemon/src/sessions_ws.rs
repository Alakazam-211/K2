...
pub async fn serve_session_subscribe_connection(
    ...
    // left streaming session frames until they happen to disconnect.
    ...
    // a revoked connect-user (whose token `revoke_user_sessions` drops
    // from the in-memory session map) would otherwise keep receiving live
    // session frames until they happened to disconnect. The owner token
    // is never revoked, so owner connections sail through the compare.
        ...
        tokio::select! {
            ...
            _ = auth_recheck.tick() => {
                if !crate::routes::http::token_still_valid(&token, &owner_token) {
                    log_debug!(
                        "[daemon/sessions_ws] token revoked mid-session; \
                         closing subscribe WS for session {session_id}"
                    );
