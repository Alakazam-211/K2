...
const REQUIRED_COLOR_SLOTS = [
  ...
  'status-ok-soft', 'status-warn-soft', 'status-error-soft', 'neutral', 'control-track-off',
...
const registry = styles.map((s) => {
  ...
  // Optional WebGL text-weight override from style.json. Omitted when
  // unset so the runtime derives from terminal bg luminance.
  if (typeof s.manifest.terminalTextGamma === 'number' && Number.isFinite(s.manifest.terminalTextGamma)) {
    entry.terminalTextGamma = s.manifest.terminalTextGamma
  }
  return entry
})
