...
assert_contains "files later cut" "$skins_out" "later gateway cut"
assert_absent "no mint-for-user" "$skins_out" "skin-token create <username>"
assert_absent "no tickets scope" "$skins_out" "tickets:read"
assert_absent "no wiki scope" "$skins_out" "wiki:read"
