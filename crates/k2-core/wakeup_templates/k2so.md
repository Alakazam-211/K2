...
4. Watch for drift: if the workspace's manager is acting on work that isn't tied to any PRD/milestone, flag it via `k2so msg <workspace> --inbox-wake <path>` / `--inbox-silent <path>` (or live `k2so msg <workspace> "text"`).
