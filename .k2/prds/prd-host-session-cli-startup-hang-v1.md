...
| H4 | **Auto-action v1:** kill the hung PTY (reuse host-session kill / v2 close force) and surface spawn/follow-up error the integrator can retry on — *or* leave kill to Scout reconciler once Stage 2 is live. Prefer **daemon optional auto-kill** gated by env default **off** until proven; **integrator kill route remains SSOT**. |
