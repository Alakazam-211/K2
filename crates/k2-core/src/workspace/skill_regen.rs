...
pub fn compose_source_paths(project_path: &str) -> Vec<PathBuf> {
    use crate::workspace::agent_identity::{agent_dir, find_primary_agent, workspace_agent_path};
    ...
    out.push(workspace_agent_path(project_path).join("AGENT.md"));
    if let Some(primary) = find_primary_agent(project_path) {
        let p = agent_dir(project_path, &primary).join("AGENT.md");
...
fn compose_agents_md(project_path: &str) -> String {
    ...
    let mut out = format!(
        "# {project_name}\n\n\
         ...
         (AGENT.md + PROJECT.md + Tooling) and optional context layers. \
         Edit those source files (the daemon watches them and recomposes) \
         or run `k2 agent context regen`. Do not edit this file — harness \
         mirrors (CLAUDE.md, GEMINI.md, .cursor/rules) symlink to it. -->\n\n"
    );
...
mod tests {
    ...
    fn compose_source_paths_lists_project_and_agent_not_generated() {
        ...
        assert!(
            srcs.iter().any(|p| p.ends_with("AGENT.md")),
            "must list AGENT.md: {srcs:?}"
        );
        assert!(
