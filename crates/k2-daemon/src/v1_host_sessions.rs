...
/// `started` / `live` / `phase` / `latestSeq` / `reaper` / `durable`.
///
...
///
/// **No side effects** — does not kill, stamp reaper, inject, or evict.
pub(crate) fn handle_v1_host_status(
    ...
    // PRD §5: live OR latestSeq > 0 OR provider session file on disk.
...
mod tests {
    ...
    /// `phase=never_started`, `started=false`.
    #[test]
    ...
    /// `started` true when latestSeq > 0 even if not live → `finished`.
    #[test]
    ...
    fn status_phase_live_is_working_and_started() {
        ...
        let started = true || 0 > 0 || false; // live || seq || file
