...
interface AddWorkspaceDialogState {
  ...
  onConfirm: ((opts: { seedWiki: boolean; seedAgentsMd: boolean; fanout: boolean }) => Promise<void>) | null

  open: (args: {
    path: string
    onConfirm: (opts: { seedWiki: boolean; seedAgentsMd: boolean; fanout: boolean }) => Promise<void>
  }) => void
  close: () => void
