...
/// The signature line stamped into every K2-generated copy so the probe
/// can tell a managed mirror apart from a user-authored file. Same token
/// the cursor-mdc writer + the add-workspace preview use (`harness.rs`),
/// kept identical so detection is uniform across paths.
pub const K2_GENERATED_SIGNATURE: &str = "k2_generated: true";
...
pub fn detect_canonical_state(project_path: &str) -> Vec<HarnessProbe> {
    ...
    // Leftover list (HARNESS_WORKSPACE_FILES). Labels mirror the
    // onboarding probe table where the names overlap. Cwd AGENTS.md
    // is probed via HARNESS_PROBE_TARGETS (generate-owned).
...
fn probe_file_state(abs: &Path) -> HarnessFileState {
    ...
    match fs::read_to_string(abs) {
        Ok(content)
            if content.contains(K2_GENERATED_SIGNATURE)
                || content.contains(crate::workspace::onboarding::AGENTS_MD_COMPOSE_BANNER) =>
        {
...
mod tests {
    ...
    #[test]
    fn detect_treats_compose_banner_agents_md_as_unified() {
        let proj = scratch_project();
        fs::write(
            proj.join("AGENTS.md"),
            "<!-- GENERATED by K2 at 2026-08-17T00:00:00Z from system sources -->\n\n# ws\n",
        )
        .unwrap();
        let probes = detect_canonical_state(proj.to_str().unwrap());
        let agents = probes
            .iter()
            .find(|p| p.relative_path == "AGENTS.md")
            .unwrap_or_else(|| panic!("AGENTS.md must be probed, got {probes:?}"));
        assert_eq!(
            agents.state,
            HarnessFileState::Unified {
                form: UnifiedForm::Copy
            },
            "compose-banner AGENTS.md is generate-owned, not Unmanaged"
        );
        fs::remove_dir_all(&proj).ok();
    }
