...
assert_eq "remote-session → remote-session" "$(_cli_tool_id_for_verb remote-session)" "remote-session"
...
assert_eq "domain → dns tool" "$(_cli_tool_id_for_verb domain)" "dns"
assert_eq "cert → dns tool" "$(_cli_tool_id_for_verb cert)" "dns"
