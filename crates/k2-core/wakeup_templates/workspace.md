...
2. Triage your inbox (`k2so inbox`). For each item, decide: act on it yourself, file it for later (`k2so inbox move <id> <folder>`), or — if it should go elsewhere — forward via `k2so msg <workspace> --inbox-wake <path>` / `--inbox-silent <path>` (recipient opens with `k2so inbox read <id>`).
