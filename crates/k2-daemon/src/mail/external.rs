...
/// A missing row must never be mistaken for oauth — defaulting to
/// password keeps LOGIN, which is the add-time path and the pre-O2 path.
/// A schema without the columns is still loud.
