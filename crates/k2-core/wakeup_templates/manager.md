<!-- DEFAULT TEMPLATE — K2 scaffolded this for a workspace primary agent in manager mode.
...
2. Triage your inbox (`k2so inbox`) in priority order. For each item:
   ...
   - If it's ambiguous, ask the sender for clarification: `k2so msg <workspace> "question..."` (live) or `k2so msg <workspace> --inbox-wake <path>` / `--inbox-silent <path>` (tray package; recipient opens with `k2so inbox read <id>`).
