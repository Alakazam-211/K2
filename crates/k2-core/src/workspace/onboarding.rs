...
pub fn set_agents_md_generate_enabled(project_path: &str, enabled: bool) -> Result<(), String> {
    ...
    if enabled {
        ...
        atomic_write_str(&flag, "")
            .map_err(|e| format!("write agents-md-generate flag: {e}"))?;
    } else if flag.exists() {
...
mod tests {
    ...
    fn plant_leaves_legacy_our_symlink() {
        ...
        fs::write(&canonical, format!("{AGENTS_MD_COMPOSE_BANNER} -->\ncanon\n")).unwrap();
        ...
        assert!(meta.file_type().is_symlink(), "legacy our-symlink must stay a symlink");
        fs::remove_dir_all(&p).ok();
    }
