...
- **Cert issue vs CNAME.** If the apex is `dns_write`, `k2 cert issue` uses DNS-01 on `_acme-challenge.<host>` (that name is not the `mail` CNAME). It waits until 8.8.8.8/1.1.1.1 see the TXT before telling Let's Encrypt (ns1 can be ahead). If the hostname is a CNAME and the zone is not `dns_write`, it fails loud instead of HTTP-01 following the CNAME onto a closed :80.
